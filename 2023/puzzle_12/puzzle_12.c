...
int main(void) {
  ...
  while (getline(&line, &len, fptr) != -1) {
    ...
    for (int i = 0; i < 4; i++) {
      if (!line_idx) {
        strcat(time, results[i]);
        continue;
      }
      strcat(distance, results[i]);
    }
    line_idx++;
  }
  ...
  char* ptr;
  int num_ways = 0;
  uint64_t target_distance = strtoull(distance, &ptr, 10);
  uint64_t time_num = strtoull(time, &ptr, 10);

  printf("time: %s\n", time);
  printf("dist: %s\n", distance);
  for (uint64_t j = 0; j <= time_num; j++) {
    uint64_t distance = (time_num - j) * j;
