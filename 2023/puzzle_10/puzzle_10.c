...
int main(void) {
    ...
    switch (section_id) {
      ...
      case 0:
        for (int i = 0; i < 25; i++) {
          ...
          if (i % 2 != 0) {
            seed_range r = {
              .begin_id=strtoull(results[i-1], &ptr, 10),
              .range=strtoull(results[i], &ptr, 10)
            };
            seed_ids[i] = r;
            num_seeds++;
          }
  ...
  for (int i = 0; i < num_seeds; i++) {
    seed_range seed = seed_ids[i];
    for (uint64_t seed_id = seed.begin_id; seed_id < seed.begin_id + seed.range; seed_id++) {
