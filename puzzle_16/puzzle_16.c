...
int main(void) {
  ...
  while (getline(&line, &len, fptr) != -1) {
    ...
    if (results[0][2] == 'A') {
      strcpy(current_nodes[begin_idx], results[0]);
      begin_idx++;
    }
  ...
  uint64_t num_steps[6] = {0};
  for (int i = 0; i < 6; i++) {
    int step_idx = 0;
    while (current_nodes[i][2] != 'Z') {
        fork* f = map_lookup(current_nodes[i]);
        if (steps[step_idx] == 'L') {
          strncpy(current_nodes[i], f->left, 4);
        } else {
          strncpy(current_nodes[i], f->right, 4);
        }
      step_idx++;
  ...
  uint64_t total_num_steps = lcm(num_steps[0], num_steps[1]);
  for (int i = 2; i < 6; i++)
    total_num_steps = lcm(total_num_steps, num_steps[i]);
