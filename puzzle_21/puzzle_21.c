...
ColumnNode* get_column_node(int idx) {
  ColumnNode* curr = INIT_COLUMN_NODE;
  while (curr) {
    if (curr->idx == idx)
      return curr;
...
int main(void) {
  ...
  int shortest_len_sum = 0;
  while (current) {
    GalaxyNode* secondary = current;
    while (secondary) {
      if (secondary == current) {
        secondary = secondary->next;
        continue;
      }
      shortest_len_sum += abs(current->x - secondary->x) + abs(current->y - secondary->y);
      secondary = secondary->next;
    }
