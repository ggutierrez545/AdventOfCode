...
bool assess_node(uint64_t **values, uint64_t target, uint64_t current_value, int next_idx) {
  ...
  char cat_value[20];
  sprintf(cat_value, "%li%li", current_value, *values[next_idx]);
  char *endptr;
  uint64_t third_value = strtoll(cat_value, &endptr, 10);
  ...
  if (res2) return true;
  //printf("Assessing: %li || %li = %li\n", current_value, *values[next_idx], third_value);
  bool res3 = assess_node(values, target, third_value, next_idx + 1);
  if (res3) return true;
